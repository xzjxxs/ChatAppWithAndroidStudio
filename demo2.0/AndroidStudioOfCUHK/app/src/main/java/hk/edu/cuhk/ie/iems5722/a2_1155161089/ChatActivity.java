...
public class ChatActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String url = "http://18.116.63.13/api/a3/get_messages?chatroom_id=" + chatroomId + "&page=1";
            ...
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                if (isFirst && scrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE) {
                    if (next_page <= total_pages) {
                        ...
                        String newUrl = "http://18.116.63.13/api/a3/get_messages?chatroom_id=" + chatroomId + "&page=" + next_page;
                        Log.e("newUrl", newUrl);
                        listMsgTask task = new listMsgTask();
                        task.execute(newUrl);
        ...
        send.setOnClickListener(v -> {
            ...
            if (!"".equals(sendMsg) && sendMsg.length() < 200) {
                ...
                // perform background operation
                String sendUrl = "http://18.116.63.13/api/a3/send_message";
                sendMsgTask sendTask = new sendMsgTask();
                sendTask.execute(sendUrl, chatroomId, "1155161089", "Xu", sendMsg);

                // refresh
                String newUrl = "http://18.116.63.13/api/a3/get_messages?chatroom_id=" + chatroomId + "&page=1";
